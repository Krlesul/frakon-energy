...
class ExecutionSafetyItem:
    attempt_id: str; lifecycle_id: str; lifecycle_state: str; entity_id: str; current_state: str | None
    service_call_performed: bool | None; stop_ownership_required: bool; stop_ownership_ready: bool
    stop_ownership_reason: str; stop_lifecycle_state: str | None; stop_scheduler_status: str | None
    safety_status: str; unsafe_reason: str | None
    def as_dict(self) -> dict[str, Any]: return asdict(self)


def _live_state(hass: HomeAssistant, entity_id: str) -> str | None:
    state = hass.states.get(entity_id); return str(state.state).strip().lower() if state is not None else None
...
async def _item(hass: HomeAssistant, *, entry_id: str, record: ExecutionLifecycleRecord, scheduler_status_by_start: dict[str, str]) -> ExecutionSafetyItem:
    ...
    return ExecutionSafetyItem(
        attempt_id=record.attempt_id, lifecycle_id=record.lifecycle_id, lifecycle_state=record.state,
        entity_id=record.entity_id, current_state=_live_state(hass, record.entity_id),
        service_call_performed=record.as_dict()["service_call_performed"], stop_ownership_required=required,
        stop_ownership_ready=proof.ownership_ready, stop_ownership_reason=proof.reason,
        ...
        safety_status="unsafe" if unsafe_reason else "safe", unsafe_reason=unsafe_reason,
    )
...
def _capacity_guard_summary(capacity: Any) -> dict[str, Any]:
    ...
    data_ready = not guard_active or (configured and capacity.topology_ready and capacity.source_available and capacity.status not in {STATUS_TOPOLOGY_NOT_READY, STATUS_SOURCE_UNAVAILABLE})
    current_limit_exceeded = guard_active and capacity.status == STATUS_OVER_LIMIT
    if not guard_active: blocking_reason = None
    elif not configured: blocking_reason = "site_capacity_limit_not_configured"
    elif not capacity.topology_ready or capacity.status == STATUS_TOPOLOGY_NOT_READY: blocking_reason = "site_capacity_topology_not_ready"
    elif not capacity.source_available or capacity.status == STATUS_SOURCE_UNAVAILABLE: blocking_reason = "site_capacity_source_unavailable"
    elif current_limit_exceeded: blocking_reason = "site_capacity_already_over_limit"
    else: blocking_reason = None
    return {
        "configured": configured, "guard_active": guard_active, "data_ready": data_ready,
        "currently_blocks_all_new_starts": blocking_reason is not None, "blocking_reason": blocking_reason,
        "status": capacity.status, "reason": capacity.reason, "max_grid_import_kw": capacity.max_grid_import_kw,
        "current_grid_import_kw": capacity.current_grid_import_kw, "grid_headroom_kw": capacity.grid_headroom_kw,
        "grid_over_limit_kw": capacity.grid_over_limit_kw, "utilization_percent": capacity.utilization_percent,
...
async def async_execution_safety_status(hass: HomeAssistant, *, entry_id: str) -> dict[str, Any]:
    ...
    if entry is None: raise ValueError(f"config entry not found: {entry_id}")
    ...
    start_recovery = lifecycle_recovery_summary(hass, entry_id); stop_recovery = stop_recovery_summary(hass, entry_id)
    stop_runtime = stop_scheduler(hass, entry_id); start_runtime = start_scheduler(hass, entry_id); pending_runtime = pending_run_scheduler(hass, entry_id)
    arm_status = await async_execution_arm_status(hass, entry_id)
    stop_scheduler_statuses = stop_runtime.statuses(); start_scheduler_statuses = start_runtime.statuses(); pending_scheduler_statuses = pending_runtime.statuses()
    ...
    return {
        "entry_id": entry_id, "execution_arm": arm_status, "site_capacity": capacity.as_dict(), "site_capacity_guard": capacity_guard,
        "start_recovery": start_recovery.as_dict(), "stop_recovery": stop_recovery.as_dict(),
        ...
        "start_runtime_ready": start_recovery_ready, "stop_runtime_ready": stop_runtime_ready,
        "autonomous_start_runtime_ready": autonomous_start_runtime_ready, "pending_run_runtime_ready": pending_run_runtime_ready,
        "execution_armed": execution_armed, "explicit_start_executor_available": explicit_start_executor_available,
        "explicit_stop_executor_available": True, "autonomous_stop_enabled": stop_runtime_ready,
        "autonomous_start_enabled": autonomous_start_runtime_ready and execution_armed,
        "autonomous_pending_run_enabled": pending_run_runtime_ready, "unsafe_start_lifecycles": unsafe,
        "items": [item.as_dict() for item in items], "read_only": True, "state_transition_performed": False,
