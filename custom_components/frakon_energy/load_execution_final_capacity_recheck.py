...
from .load_execution_capacity_reservation import (
    CapacityReservation,
...
def _result(
    *,
    ...
    reserved_other_power_kw: float = 0.0,
    effective_planned_power_kw: float | None = None,
    active_reservations: tuple[CapacityReservation, ...] = (),
    reservation: CapacityReservation | None = None,
    capacity_gate: SiteCapacityGateDecision | None = None,
    ...
    return FinalCapacityRecheck(
        ...
        capacity_gate=capacity_gate.as_dict() if capacity_gate is not None else None,
        can_start=can_start,
    ...
    *,
    ...
    try:
        ...
        raise FinalCapacityRecheckError(
            f"capacity reservation state unavailable: {err}"
        ) from err
    ...
    decision: SiteCapacityGateDecision = evaluate_site_capacity_execution_gate(
        capacity=capacity,
        planned_power_kw=effective_planned,
    )
    ...
    if not allowed:
        return _result(
            ...
            lifecycle=lifecycle,
            reserved_other_power_kw=reserved_other,
            effective_planned_power_kw=effective_planned,
            active_reservations=active,
            capacity_gate=decision,
        )
    ...
    try:
        reservation, created = await repository.async_reserve(
            ...
            power_kw=lifecycle.plan.power_kw,
            now=now_ts,
        )
    except CapacityReservationError as err:
        raise FinalCapacityRecheckError(
            f"capacity reservation could not be persisted: {err}"
        ) from err
    except Exception as err:
        raise FinalCapacityRecheckError(
            f"capacity reservation persistence unavailable: {err}"
        ) from err

    return _result(
        status=FINAL_RECHECK_READY,
        reason=decision.reason,
        capacity=capacity,
        can_start=True,
        guard_active=decision.guard_active,
