...
class VisionQHistoryStore:
    ...
    async def async_record(self, measurement: VisionQMeasurement) -> bool:
        ...
        return await self.async_import_snapshots(
            (
                MeterSnapshot(
                    captured_at=captured,
                    high_rate_kwh=Decimal(str(measurement.high_rate_kwh)),
                    low_rate_kwh=Decimal(str(measurement.low_rate_kwh)),
                ),
            )
        ) > 0
    ...
    def latest_timestamp(self) -> int | None:
        ...
        if not readings:
            return None
        return int(readings[-1].captured_at.timestamp())
    ...
    async def async_import_snapshots(self, snapshots: tuple[MeterSnapshot, ...]) -> int:
        """Merge provider history, retaining the latest reading for each local day."""
        imported = 0
