...
# Confirmed by the official 2026 ERÚ/OTE publications. These are the only
# monetary values hardcoded here; table-/amendment-dependent values stay parser
# inputs and must never be guessed.
POZE_2026_CZK_PER_KWH = Decimal("0")
OTE_NON_NETWORK_2026_CZK_PER_MONTH = Decimal("12.87")

# ERÚ published a later amendment to price measure 13/2025. It is intentionally
# kept as an explicit reference rather than silently folded into the January
# baseline; a version-aware parser must decide its applicability and values.
...
class CzechRegulatedTariffInputs:
    """Fully parsed Czech regulated inputs before conversion to FRAKON components.
