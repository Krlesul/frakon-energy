...
class SiteCapacitySettings:
    ...
    def from_options(cls, options: Mapping[str, Any]) -> "SiteCapacitySettings":
        ...
        return cls(
            ...
            execution_guard_enabled=raw.get(CONF_EXECUTION_GUARD_ENABLED) is True,
        )


def update_site_capacity_limit(options: Mapping[str, Any], max_grid_import_kw: float | None) -> dict[str, Any]:
    ...
    settings = SiteCapacitySettings(max_grid_import_kw=max_grid_import_kw, execution_guard_enabled=existing.execution_guard_enabled).validated()
    updated = dict(options)
    updated[OPTION_SITE_CAPACITY] = settings.as_dict()
    return updated


def update_site_capacity_guard(options: Mapping[str, Any], enabled: bool) -> dict[str, Any]:
    existing = SiteCapacitySettings.from_options(options)
    settings = SiteCapacitySettings(max_grid_import_kw=existing.max_grid_import_kw, execution_guard_enabled=bool(enabled)).validated()
    updated = dict(options)
    updated[OPTION_SITE_CAPACITY] = settings.as_dict()
    return updated
...
def build_site_capacity_status(hass: HomeAssistant, *, entry_id: str, options: Mapping[str, Any]) -> SiteCapacityStatus:
    ...
    if limit is None:
        return SiteCapacityStatus(entry_id, STATUS_NOT_CONFIGURED, False, flow.topology.get(CONF_GRID_METER_SCOPE) == "whole_house", grid.value_kw is not None, None, abs(grid.value_kw) if grid.value_kw is not None else None, None, None, None, grid.entity_id, "Maximální odběr ze sítě není nastavený.", execution_guard_active=guard_active)
    if flow.topology.get(CONF_GRID_METER_SCOPE) != "whole_house":
        return SiteCapacityStatus(entry_id, STATUS_TOPOLOGY_NOT_READY, True, False, grid.value_kw is not None, limit, abs(grid.value_kw) if grid.value_kw is not None else None, None, None, None, grid.entity_id, "Kapacitní ochrana vyžaduje potvrzené hlavní měření celého domu.", execution_guard_active=guard_active)
    if grid.value_kw is None:
        return SiteCapacityStatus(entry_id, STATUS_SOURCE_UNAVAILABLE, True, True, False, limit, None, None, None, None, grid.entity_id, f"Aktuální odběr ze sítě není použitelný ({grid.reason}).", execution_guard_active=guard_active)
    ...
    reason = f"Odběr překračuje nastavený limit o {over:.3f} kW." if over > 0 else f"Do nastaveného limitu zbývá {headroom:.3f} kW."
    return SiteCapacityStatus(entry_id, status, True, True, True, limit, current, headroom, over, utilization, grid.entity_id, reason, execution_guard_active=guard_active)
