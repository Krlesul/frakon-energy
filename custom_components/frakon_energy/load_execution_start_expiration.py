...
no-dispatch outcome so autonomous polling does not repeatedly reconsider stale
work. It performs no Home Assistant service call.
"""
...
def _is_expiration_terminal(record: Any) -> bool:
    return (
...
async def async_expire_prepared_start(
    ...
    *,
    ...
    async with _transaction_lock(hass, entry_id):
        ...
        if _is_expiration_terminal(record):
            return {
