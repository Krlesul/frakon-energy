...
class ExecutionActionSnapshot:
    """Deeply immutable description of the only action allowed for an attempt."""
    ...
    def from_attempt_and_intent(
        ...
        *,
        ...
        """Bind one validated consumed attempt to one fixed allowlisted action."""
        attempt.validated()
        intent.validated()
...
class ActionSnapshotRevalidation:
    """Read-only revalidation of persisted binding against current runtime state."""
...
def revalidate_action_snapshot(
    ...
    *,
    ...
    """Recheck immutable attempt/action binding without executing anything."""
    snapshot.validated()
...
class ExecutionActionSnapshotLedger:
    """Validated immutable snapshot ledger keyed by execution attempt."""
...
class ExecutionActionSnapshotRepository:
    """Persistent idempotent repository with rollback on storage failure."""
...
def home_assistant_action_snapshot_repository(
    ...
    """Create one persistent action-snapshot repository for a config entry."""
    store = Store(
        hass,
