...
def evaluate_site_capacity_execution_gate(*, capacity: SiteCapacityStatus, planned_power_kw: float) -> SiteCapacityGateDecision:
    if isinstance(planned_power_kw, bool) or not math.isfinite(planned_power_kw) or planned_power_kw <= 0:
        return SiteCapacityGateDecision(
            status=CAPACITY_GATE_BLOCKED, reason=REASON_INVALID_PLAN_POWER,
            capacity_status=capacity.status, configured=capacity.configured,
            planned_power_kw=float(planned_power_kw), current_grid_import_kw=capacity.current_grid_import_kw,
            max_grid_import_kw=capacity.max_grid_import_kw, grid_headroom_kw=capacity.grid_headroom_kw,
            projected_grid_import_kw=None, projected_over_limit_kw=None, can_start=False,
    ...
    if not capacity.execution_guard_active:
        return SiteCapacityGateDecision(
            status=CAPACITY_GATE_BYPASSED, reason=REASON_GUARD_DISABLED,
            projected_grid_import_kw=(capacity.current_grid_import_kw + planned_power_kw if capacity.current_grid_import_kw is not None else None),
            projected_over_limit_kw=None, can_start=True, **base,
        )
    if capacity.status == STATUS_NOT_CONFIGURED or not capacity.configured:
        return SiteCapacityGateDecision(status=CAPACITY_GATE_BLOCKED, reason=REASON_LIMIT_MISSING, projected_grid_import_kw=None, projected_over_limit_kw=None, can_start=False, **base)
