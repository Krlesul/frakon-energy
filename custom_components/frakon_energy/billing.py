...
def next_default_settlement_date(reference_date: date) -> date:
    candidate = date(reference_date.year, DEFAULT_SETTLEMENT_MONTH, DEFAULT_SETTLEMENT_DAY)
    if candidate < reference_date:
        candidate = date(reference_date.year + 1, DEFAULT_SETTLEMENT_MONTH, DEFAULT_SETTLEMENT_DAY)
    return candidate
...
class BillingCycle:
    ...
    @classmethod
    def with_default_settlement_date(cls, *, start_date: date, baseline: MeterBaseline) -> BillingCycle:
        return cls(start_date=start_date, expected_settlement_date=next_default_settlement_date(start_date), baseline=baseline)
...
class BillingCalculator:
    @classmethod
    def calculate(cls, *, cycle: BillingCycle, as_of: date, advances: Iterable[AdvancePeriod], accrued_cost_czk: Decimal, projected_total_cost_czk: Decimal) -> BillingSnapshot:
        ...
        projected_paid = cls._sum_advances(cycle.start_date, cycle.expected_settlement_date, periods)
        current_balance = paid - accrued_cost_czk
        projected_balance = projected_paid - projected_total_cost_czk
        remaining_months = cls._count_months(cls._next_month(as_of), cycle.expected_settlement_date)
        remaining_cost = max(Decimal("0"), projected_total_cost_czk - paid)
        recommended = remaining_cost / remaining_months if remaining_months > 0 else max(Decimal("0"), -projected_balance)
