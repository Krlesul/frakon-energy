...
export function SiteCapacitySettings({ hass }: { hass?: HomeAssistant }) {
  ...
  return <article className={`chart-card technology-settings site-capacity-settings ${guardActive ? "is-guarded" : ""}`}>
    ...
    <p className="settings-copy">Zadej skutečný maximální odběr v kW, který chceš pro dům respektovat. FRAKON ho neodhaduje z jističe ani napětí. Samotný limit je diagnostický; teprve samostatný execution guard jej použije jako fail-closed podmínku pro nové fyzické starty.</p>
    ...
    <div className="role-list">
      ...
      <div className="role-row">
        <div className="role-row__label"><b>Execution guard kapacity</b><small>{guardActive ? "Nový turn_on musí projít kontrolou aktuálního odběru + plánovaného výkonu. Stop povinnosti zůstávají nezávislé." : "Vypnuto: limit je pouze diagnostický a fyzické starty neblokuje."}</small></div>
        <label className="technology-toggle">
          <input type="checkbox" checked={guardActive} disabled={busy || !status?.configured} onChange={(event) => void setGuard(event.target.checked)} />
          <span>{guardActive ? "Aktivní" : "Vypnutý"}</span>
        </label>
    ...
    {status ? <div className="discovery-summary">
      ...
      <span>Execution guard <b>{guardActive ? "aktivní" : "vypnutý"}</b></span>
    </div> : null}
