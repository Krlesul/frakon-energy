...
export function LoadExecutionPolicyPanel({ hass, entryId, profiles, earliestStart, deadline }: { hass?: HomeAssistant; entryId: string | null; profiles: PolicyProfile[]; earliestStart: string; deadline: string }) {
  ...
  return <section className="load-policy-panel">
    ...
    <p>Žádný režim Automatic v policy neexistuje. Výsledek může být pouze <b>Blocked</b> nebo <b>Approval required</b>. Tato karta umí vytvořit krátkodobý approval artifact, ale sama nemá tlačítko Start. Případná fyzická exekuce musí následně projít consume/attempt, immutable action, readiness, lifecycle, stop lease, bounded gate a globálním ARM interlockem.</p>
    ...
    {issuedApproval ? <div className="load-issued-approval"><div className="load-issued-approval__header"><div><span className="eyebrow">Approval artifact · vydán</span><h4>Krátkodobé schválení scope</h4></div><span>bez exekuce</span></div><div className="load-issued-approval__grid"><div><span>Approval ID</span><code>{issuedApproval.approval_id}</code></div><div><span>Platnost</span><b>{formatEpoch(issuedApproval.issued_at)} → {formatEpoch(issuedApproval.expires_at)}</b></div><div><span>TTL</span><b>{issuedApproval.ttl_seconds} s</b></div><div><span>HMAC podpis</span><b>vydán · skrytý v UI</b></div></div><div className="load-approval-scope__digest"><span>Svázaný snapshot digest</span><code>{issuedApproval.snapshot_digest}</code></div><p>Artifact je pouze schválení přesného scope. Pro případnou exekuci musí být explicitně spotřebován do persistentního attemptu a následně projít immutable action, readiness, lifecycle, stop lease, bounded gate a ARM interlockem. Tato karta nemá tlačítko Start.</p><small>approval_issued={String(issuedApproval.approval_issued)} · consumed={String(issuedApproval.consumed)} · execution_performed={String(issuedApproval.execution_performed)} · executor_available={String(issuedApproval.executor_available)}</small></div> : null}
  </section>;
}
