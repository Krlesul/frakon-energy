...
function readDashboardState(hass?: HomeAssistant): DashboardState {
  ...
  const tariffEntity = findByAliases(hass, ["frakon energy tariff", "aktualni tarif", "cez hdo aktivni interval", "nizky tarif aktivni", "vysoky tarif aktivni"]); const ntActive = findByAliases(hass, ["nizky tarif aktivni"]); const countdownEntity = findByAliases(hass, ["frakon energy countdown", "cez hdo odpocet", "cez hdo dalsi zmena odpocet", "nizky tarif zbyva", "vysoky tarif zbyva"]); const nextSwitchEntity = findByAliases(hass, ["frakon energy next switch", "cez hdo dalsi zmena", "nizky tarif konec", "nizky tarif zacatek"]); const scheduleEntity = findByAliases(hass, ["frakon energy schedule", "cez hdo dnesni rozvrh", "hdo rozvrh"]); const schedules = splitSchedule(scheduleEntity);
...
export async function findFrakonEnergyEntryId(hass: HomeAssistant): Promise<string | null> { const entries = await callHomeAssistantWs<Array<{ entry_id: string; domain?: string }>>(hass, { type: "config_entries/get" }); return entries.find((entry) => entry.domain === "frakon_energy")?.entry_id ?? null; }
