...
type PrimaryEntrySummary = {
  entry_id: string;
  title: string;
  provider: "visionq";
  loaded: true;
  visionq_entry_count: number;
  loaded_visionq_entry_count: number;
};
...
function hasStructuredSchedule(entity: HassEntity | undefined): boolean { if (!entity) return false; return parseRawSchedule(entity.attributes.schedule).length > 0 || parseRawSchedule(entity.attributes.today_schedule).length > 0 || parseRawSchedule(entity.attributes.dnesni_rozvrh).length > 0 || parseRawSchedule(entity.attributes.intervals).length > 0 || parseRawSchedule(entity.attributes.raw_data).length > 0; }
function findStructuredScheduleEntity(hass: HomeAssistant): HassEntity | undefined { return allStates(hass).find((entity) => hasStructuredSchedule(entity)); }
...
function scheduleTariffAt(items: ScheduleItem[], now: Date): "NT" | "VT" | null { const current = items.find((item) => { const start = new Date(item.start); const end = new Date(item.end); return !Number.isNaN(start.getTime()) && !Number.isNaN(end.getTime()) && start.getTime() <= now.getTime() && end.getTime() > now.getTime(); }); return current?.tariff ?? null; }
function nextScheduleTransition(today: ScheduleItem[], tomorrow: ScheduleItem[], now: Date): Date | null { const items = [...today, ...tomorrow].map((item) => ({ start: new Date(item.start), end: new Date(item.end) })).filter((item) => !Number.isNaN(item.start.getTime()) && !Number.isNaN(item.end.getTime())).sort((a, b) => a.start.getTime() - b.start.getTime()); const current = items.find((item) => item.start.getTime() <= now.getTime() && item.end.getTime() > now.getTime()); if (current) return current.end; const next = items.find((item) => item.start.getTime() > now.getTime()); return next?.start ?? null; }
...
function dateIsPast(value: string | null): boolean { if (!value) return false; const parsed = new Date(`${value}T23:59:59`); if (Number.isNaN(parsed.getTime())) return false; return parsed.getTime() < Date.now(); }

function readDashboardState(hass?: HomeAssistant): DashboardState {
  if (!hass) return EMPTY_STATE;
  const tariffEntity = findByAliases(hass, ["hdo tarif", "frakon energy tariff", "aktualni tarif", "cez hdo aktivni interval", "nizky tarif aktivni", "vysoky tarif aktivni"]); const ntActive = findByAliases(hass, ["cez_hdo_lowtariffactive", "nizky tarif aktivni"]); const countdownEntity = findByAliases(hass, ["hdo odpocet", "frakon energy countdown", "cez hdo odpocet", "cez hdo dalsi zmena odpocet", "nizky tarif zbyva", "vysoky tarif zbyva"]); const nextSwitchEntity = findByAliases(hass, ["hdo dalsi prepnuti", "frakon energy next switch", "cez hdo dalsi zmena", "nizky tarif konec", "nizky tarif zacatek"]); const scheduleAliasEntity = findByAliases(hass, ["hdo dnesni rozvrh", "frakon energy schedule", "cez hdo dnesni rozvrh", "hdo rozvrh", "cez_hdo_schedule"]); const scheduleEntity = findStructuredScheduleEntity(hass) ?? (hasStructuredSchedule(scheduleAliasEntity) ? scheduleAliasEntity : undefined); const schedules = splitSchedule(scheduleEntity); const now = new Date();
  let tariff: "NT" | "VT" | "?" = "?"; const tariffText = tariffEntity?.state.toUpperCase() ?? ""; if (tariffText === "NT" || tariffText.includes("NIZKY")) tariff = "NT"; else if (tariffText === "VT" || tariffText.includes("VYSOKY")) tariff = "VT"; else if (ntActive?.state === "on" || ntActive?.state.toLowerCase() === "true") tariff = "NT"; else if (ntActive?.state === "off" || ntActive?.state.toLowerCase() === "false") tariff = "VT"; else tariff = scheduleTariffAt(schedules.today, now) ?? "?";
  const nextTimestamp = nextSwitchEntity?.state; const candidateNextDate = nextTimestamp && !["unknown", "unavailable"].includes(nextTimestamp) ? new Date(nextTimestamp) : null; const entityNextDate = candidateNextDate && !Number.isNaN(candidateNextDate.getTime()) && candidateNextDate.getTime() > now.getTime() ? candidateNextDate : null; const nextDate = entityNextDate ?? nextScheduleTransition(schedules.today, schedules.tomorrow, now); const entityCountdown = parseCountdown(countdownEntity?.state); const scheduleCountdown = nextDate ? Math.max(0, Math.floor((nextDate.getTime() - now.getTime()) / 1000)) : null; const countdownSeconds = entityCountdown ?? scheduleCountdown; const highRateKwh = numberState(findByAliases(hass, ["frakon energy high rate", "vt celkem"])); const lowRateKwh = numberState(findByAliases(hass, ["frakon energy low rate", "nt celkem"])); const monthlyAdvanceCzk = numberState(findByAliases(hass, ["billing monthly advance", "mesicni zaloha", "stala mesicni platba"])); const settlementDate = textState(findByAliases(hass, ["billing settlement date", "predpokladane vyuctovani", "datum vyuctovani"])); const missingSetup: string[] = []; if (monthlyAdvanceCzk === null) missingSetup.push("Měsíční záloha"); if (settlementDate === null) missingSetup.push("Datum vyúčtování"); if (findByAliases(hass, ["pocatecni stav vt", "baseline high rate"]) === undefined) missingSetup.push("Počáteční stav VT"); if (findByAliases(hass, ["pocatecni stav nt", "baseline low rate"]) === undefined) missingSetup.push("Počáteční stav NT"); const lastEntity = findByAliases(hass, ["posledni aktivita", "frakon energy last activity"]); const lastUpdated = lastEntity?.last_updated ?? lastEntity?.last_changed ?? null;
  return { connected: true, tariff, countdownSeconds, nextChange: nextDate ? nextDate.toLocaleTimeString("cs-CZ", { hour: "2-digit", minute: "2-digit" }) : null, todaySchedule: schedules.today, tomorrowSchedule: schedules.tomorrow, currentPrice: numberState(findByAliases(hass, ["hdo aktualni cena", "cez_hdo_currentprice", "aktualni cena", "skutecna cena", "current price"])), batteryPercent: normalizeBattery(numberState(findByAliases(hass, ["stav baterie", "battery state", "battery level"]))), highRateKwh, lowRateKwh, monthlyAdvanceCzk, paidAdvancesCzk: numberState(findByAliases(hass, ["zaplacene zalohy", "billing paid advances"])), projectedAdvancesCzk: numberState(findByAliases(hass, ["zalohy za cele obdobi", "billing projected advances"])), accruedCostCzk: numberState(findByAliases(hass, ["dosavadni naklady", "skutecne naklady", "billing accrued cost"])), currentBalanceCzk: numberState(findByAliases(hass, ["prubezny rozdil", "prubezny preplatek nedoplatek", "billing current balance"])), projectedBalanceCzk: numberState(findByAliases(hass, ["odhad preplatku nedoplatku", "billing projected balance"])), recommendedAdvanceCzk: numberState(findByAliases(hass, ["doporucena zaloha", "billing recommended advance"])), todayConsumptionKwh: numberState(findByAliases(hass, ["spotreba dnes celkem", "today consumption"])), todayCostCzk: numberState(findByAliases(hass, ["naklady dnes", "billing today cost", "today cost"])), monthConsumptionKwh: numberState(findByAliases(hass, ["spotreba tento mesic", "month consumption"])), settlementDate, billingExpired: dateIsPast(settlementDate), lastUpdated, hdoQuality: schedules.today.length > 0 || tariff !== "?" ? "live" : "fallback", missingSetup };
}
