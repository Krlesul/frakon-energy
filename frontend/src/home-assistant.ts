...
function findStructuredScheduleEntity(hass: HomeAssistant): HassEntity | undefined { return allStates(hass).find((entity) => parseRawSchedule(entity.attributes.schedule).length > 0 || parseRawSchedule(entity.attributes.today_schedule).length > 0 || parseRawSchedule(entity.attributes.dnesni_rozvrh).length > 0); }
