...
function fallbackNtIntervals(date = new Date()): ClockInterval[] {
  const source = date.getDay() === 0 || date.getDay() === 6 ? DEFAULT_NT_SCHEDULE.weekend : DEFAULT_NT_SCHEDULE.weekday;
...
function HdoTimeline() {
  ...
  const intervals = useMemo(() => today.length > 0 ? today.map((item) => ({
  ...
  })) : fallbackNtIntervals(now), [today, now.getDay()]);
  ...
  return <article className="hdo-plan-card">
    <div className="hdo-plan-card__header"><div><span className="eyebrow">HDO plán</span><h2>Nízký tarif dnes</h2></div><DataBadge live={today.length > 0} /></div>
    ...
    <div className="visual-timeline visual-timeline--live" onPointerMove={handleInspect} onPointerLeave={() => setInspectMinutes(null)} onPointerDown={handleInspect}>
      ...
      <div className="current-time-marker" style={{ left: `${markerPosition}%` }} aria-label={`Aktuální čas ${now.toLocaleTimeString("cs-CZ", { hour: "2-digit", minute: "2-digit" })}`}><b>{now.toLocaleTimeString("cs-CZ", { hour: "2-digit", minute: "2-digit" })}</b></div>
    ...
    {today.length === 0 ? <p className="fallback-warning">Používá se náhradní plán. Časy nejsou potvrzené živými daty distributora.</p> : null}
  </article>;
}
function Overview() {
  const state = useFrakonEnergyState();
...
function BillingView() {
  const state = useFrakonEnergyState();
  return <section className="page-grid"><BillingSummary /><article className="chart-card"><span className="eyebrow">Zúčtovací období</span><h2>{state.settlementDate ? `Do ${new Date(state.settlementDate).toLocaleDateString("cs-CZ")}` : "Není kompletně nastaveno"}</h2><div className="setup-checklist">{state.missingSetup.length === 0 ? <p>Vyúčtování je připravené.</p> : state.missingSetup.map((item) => <div key={item}><span>!</span><b>{item}</b><small>Vyplňte v nastavení FRAKON Energy.</small></div>)}</div></article></section>;
}
