...
FIXED_NOW = datetime(2026, 8, 15, 18, 45, tzinfo=timezone.utc)
...
def load_module(
    *,
    ...
    def all_in_tariff_authority_from_options(options, fingerprint):
        ...
        mode = options.get("authority_mode", staged_authority)
        if mode == "missing":
    ...
    def stage_customer_tariff_proposal(options, **kwargs):
        ...
        updated["authority_mode"] = staged_authority
...
def test_manual_commercial_payload_is_strict_plain_decimal_and_only_price_authority() -> None:
    invalid_manual = (
        {
...
def test_unsafe_preview_or_wrong_staged_authority_never_persists() -> None:
    module, registered, _schemas, calls, _authority = load_module(
        preview_unsafe=True
    )
    hass = Hass(_entry())
    ...
    for authority_mode in ("verified", "missing"):
        module, registered, _schemas, calls, _authority = load_module(
            staged_authority=authority_mode
        )
        hass = Hass(_entry())
        connection = Connection()
        module.async_register_manual_customer_tariff_websocket(hass)
        asyncio.run(registered[0](hass, connection, _message()))
        assert connection.errors[0][1] == "manual_tariff_proposal_failed"
        assert any(item[0] == "stage" for item in calls)
        assert hass.config_entries.updates == []


def test_repeated_manual_stage_is_read_only_and_invalid_context_stops_early() -> None:
    module, registered, _schemas, _calls, _authority = load_module(
        already_staged=True
    )
    entry = _entry(
        options={
            "manual_proposal_saved": "a" * 64,
