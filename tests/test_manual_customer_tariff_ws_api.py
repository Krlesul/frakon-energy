...
def load_module(
    *,
    ...
    def stage_customer_tariff_proposal(options, **kwargs):
        ...
        proposal = Proposal()
        if already_staged:
            existing = dict(options)
            existing.setdefault("stored_proposal", proposal)
            existing.setdefault("authority_mode", "manual")
    ...
    def confirm_customer_tariff_proposal(options, fingerprint):
        ...
        updated = dict(options)
        updated["customer_tariff_confirmed"] = fingerprint
        return updated, proposal
    ...
    def _registry_for_hass(_hass):
        calls.append(("registry_base", base_registry))
        return base_registry

    def _registry_for_entry(hass, entry, *, registry):
        calls.append(("registry_entry", entry.entry_id, registry))
...
def test_registration_is_idempotent_and_schemas_expose_only_manual_commercial_authority() -> None:
...
def test_mnd_manual_propose_uses_entry_registry_and_server_selected_manual_authority() -> None:
    ...
    assert names.index("stage") < names.index("authority")
...
def test_manual_commercial_payload_is_strict_string_only_and_fails_before_regulator() -> None:
    invalid_payloads = (
        {
    ...
    for manual_commercial in invalid_payloads:
        module, registered, _schemas, calls, _proposal = load_module()
        hass = Hass(_entry())
        connection = Connection()
        module.async_register_manual_customer_tariff_websocket(hass)
        message = _propose_message(manual_commercial=manual_commercial)
        asyncio.run(registered[0](hass, connection, message))
        assert connection.errors[0][1] == "invalid_manual_commercial"
        assert not any(
            item[0] in {"regulated", "registry_entry", "discover", "fetch", "stage"}
            for item in calls
        )
        assert hass.config_entries.updates == []


def test_regulator_candidate_and_download_failures_never_stage_manual_prices() -> None:
    ...
    for download_mode, expected_code in (
...
def test_invalid_source_context_and_unsafe_preview_fail_closed() -> None:
    module, registered, _schemas, calls, _proposal = load_module()
    hass = Hass(_entry())
    connection = Connection()
    module.async_register_manual_customer_tariff_websocket(hass)
    ...
    assert not any(
        item[0] in {"regulated", "discover", "fetch", "stage"} for item in calls
    )
...
def test_repeated_manual_propose_has_no_write_churn_and_never_activates() -> None:
    module, registered, _schemas, _calls, Proposal = load_module(already_staged=True)
    proposal = Proposal()
    entry = _entry(
        options={
            "stored_proposal": proposal,
            "authority_mode": "manual",
            "manual_proposal_saved": proposal.fingerprint,
        }
    )
    hass = Hass(entry)
    connection = Connection()
...
def test_manual_confirm_requires_manual_authority_before_and_after_generic_confirmation() -> None:
    ...
    confirm_index = names.index("confirm")
    second_authority = names.index("authority", first_authority + 1)
    assert first_authority < confirm_index < second_authority
    assert len(hass.config_entries.updates) == 1
    payload = connection.results[0][1]
...
def test_manual_confirm_rejects_verified_or_missing_authority_before_confirmation() -> None:
    for authority_mode, expected_code in (
        ("verified", "manual_tariff_authority_mismatch"),
        ("missing", "manual_tariff_authority_not_found"),
    ):
        module, registered, _schemas, calls, Proposal = load_module(
...
def test_manual_confirm_is_idempotent_and_malformed_fingerprint_fails_closed() -> None:
    ...
    entry = _entry(
        options={
            "stored_proposal": proposal,
            "authority_mode": "manual",
            "customer_tariff_confirmed": proposal.fingerprint,
        }
    )
    hass = Hass(entry)
    connection = Connection()
    ...
    assert connection.errors == []
    assert hass.config_entries.updates == []
    assert connection.results[0][1]["confirmation_performed"] is False
