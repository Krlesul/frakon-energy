...
def test_manual_propose_is_after_common_customer_confirmation_and_before_mnd_source_and_load_plans() -> None:
    ...
    assert len(regulator) == len(customer) == len(manual) == len(mnd_source) == len(load_plan) == 1
    assert regulator[0] < customer[0] < manual[0] < mnd_source[0] < load_plan[0]
