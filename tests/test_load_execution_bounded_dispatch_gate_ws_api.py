...
def _plan() -> LoadPlan:
    ...
    return LoadPlan(
        load_id="ev-home", name="Enyaq", starts_at=START.isoformat(),
        ends_at=(START + timedelta(minutes=duration)).isoformat(), duration_minutes=duration,
        interval_count=8, power_kw=power, average_czk_kwh=average, minimum_czk_kwh=1.5,
        maximum_czk_kwh=2.5, estimated_energy_kwh=energy, estimated_cost_czk=energy * average,
    )
...
def _evidence(current_state: str = "off"):
    ...
    attempt = ExecutionAttempt(
        attempt_id="attempt-1", entry_id="entry-1", profile_id=profile.profile_id,
        entity_id=profile.entity_id, approval_id="approval-1", approval_fingerprint="a" * 64,
        snapshot_digest=execution_snapshot_digest(profile, plan, policy), intent="execute_load_plan",
...
def _capacity_status(*, active: bool = False) -> SiteCapacityStatus:
    return SiteCapacityStatus(
        entry_id="entry-1", status="within_limit", configured=True, topology_ready=True,
        source_available=True, max_grid_import_kw=12.0, current_grid_import_kw=5.0,
        grid_headroom_kw=7.0, grid_over_limit_kw=0.0, utilization_percent=41.67,
        source_entity_id="sensor.grid_in", reason="ok", execution_guard_active=active,
    )


def _capacity_decision(*, blocked: bool = False) -> SiteCapacityGateDecision:
    return SiteCapacityGateDecision(
        status=CAPACITY_GATE_BLOCKED if blocked else CAPACITY_GATE_BYPASSED,
        reason=REASON_INSUFFICIENT_HEADROOM if blocked else REASON_GUARD_DISABLED,
        capacity_status="within_limit", configured=True, planned_power_kw=11.0,
        current_grid_import_kw=5.0, max_grid_import_kw=12.0, grid_headroom_kw=7.0,
        projected_grid_import_kw=16.0, projected_over_limit_kw=4.0 if blocked else None,
        can_start=not blocked, guard_active=blocked,
    )
...
def _hass():
    return SimpleNamespace(config_entries=SimpleNamespace(async_get_entry=lambda entry_id: SimpleNamespace(options={})))
...
async def test_endpoint_requires_matching_persisted_stop_lease_without_mutation(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    lease = ExecutionStopLease.from_prepared_lifecycle(lifecycle, created_at=lifecycle.created_at)
    await repository.async_record(lease); before_saves = store.saves
