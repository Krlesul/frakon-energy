...
def _fixture(*, confirmed=True, breaker="3x25A", regulated_valid_from=date(2026, 1, 1), regulated_valid_to=date(2026, 12, 31)):
    ...
    return contracts, sources, provenance, all_in, contract, validated, parsed, regulated, evidence


def test_all_in_preview_combines_gross_supplier_and_confirmed_regulated_prices() -> None:
    _contracts, _sources, _provenance, all_in, contract, validated, parsed, regulated, evidence = _fixture()
    ...
    assert result.assembly.valid_from == date(2026, 1, 1)
    assert result.assembly.valid_to == date(2026, 12, 31)
    assert result.assembly.breaker_code == "3x25A"
    ...
    assert payload["variable_components"][0]["kind"] == "commodity"
...
def test_unconfirmed_regulated_bundle_cannot_produce_all_in_preview() -> None:
    _contracts, _sources, _provenance, all_in, contract, validated, parsed, regulated, evidence = _fixture(confirmed=False)

    with pytest.raises(ValueError, match="must be confirmed"):
        all_in.build_all_in_tariff_preview(
            download=validated,
            parsed=parsed,
            contract=contract,
            regulated=regulated,
...
def test_breaker_mismatch_fails_closed_in_existing_assembly_boundary() -> None:
    _contracts, _sources, _provenance, all_in, contract, validated, parsed, regulated, evidence = _fixture(breaker="3x32A")
...
def test_regulated_evidence_must_match_bundle_source_and_checksum() -> None:
    _contracts, sources, provenance, all_in, contract, validated, parsed, regulated, evidence = _fixture()

    wrong_source = (
        provenance.PriceEvidence(
            scope=sources.PRICE_SCOPE_REGULATED,
            source_name="ERÚ",
            document_name="wrong source",
            source_url="https://eru.gov.cz/other-official-page",
            valid_from=date(2026, 1, 1),
            valid_to=date(2026, 12, 31),
            checksum="b" * 64,
        ),
    ...
    with pytest.raises(ValueError, match="does not contain"):
        all_in.build_all_in_tariff_preview(
            ...
            contract=contract,
            regulated=regulated,
            regulated_evidence=wrong_source,
        )
    ...
    with pytest.raises(ValueError, match="checksum"):
        all_in.build_all_in_tariff_preview(
            ...
            contract=contract,
            regulated=regulated,
            regulated_evidence=wrong_checksum,
        )


def test_supplier_sha_drift_is_rejected_before_all_in_assembly() -> None:
    _contracts, _sources, _provenance, all_in, contract, validated, parsed, regulated, evidence = _fixture()
    parsed_drift = type(parsed)(
        ...
        validation_reasons=parsed.validation_reasons,
    )
...
def test_non_overlapping_regulated_validity_fails_closed() -> None:
    _contracts, _sources, _provenance, all_in, contract, validated, parsed, regulated, evidence = _fixture(
        regulated_valid_from=date(2025, 1, 1),
        regulated_valid_to=date(2025, 12, 31),
    )
