...
def load_modules():
    names = (
        ...
        "custom_components.frakon_energy.regulated_pricing", "custom_components.frakon_energy.tariff_sources",
...
def _fixture(*, confirmed=True, breaker="3x25A", regulated_valid_from=date(2026, 1, 1), regulated_valid_to=date(2026, 12, 31)):
    ...
    contract = contracts.ElectricityContract(
        supplier=contracts.Supplier.CEZ, distributor=contracts.Distributor.CEZ_DISTRIBUCE,
        product_name="Basic", contract_kind=contracts.ContractKind.INDEFINITE,
        distribution_tariff="D25d", breaker=contracts.Breaker(phases=3, amperes=25),
        valid_from=date(2026, 1, 1), valid_to=date(2026, 12, 31), customer_confirmed=False,
    )
    candidate = sources.TariffDocumentCandidate(
        document=sources.OfficialTariffDocument(
            supplier="cez", source_url="https://www.cez.cz/file/basic.pdf",
            discovered_at=datetime(2026, 8, 14, 16, 45, tzinfo=timezone.utc),
            document_date=date(2026, 1, 1), content_type="application/pdf",
        ),
        product_name="Basic", valid_from=date(2026, 1, 1), valid_to=date(2026, 12, 31),
        match_score=100, match_reasons=("exact verified fixture",),
        price_scope=sources.PRICE_SCOPE_SUPPLIER_COMMERCIAL,
    )
    content = b"%PDF-1.7\nall-in preview fixture\n%%EOF"
    sha256 = hashlib.sha256(content).hexdigest()
    document = sources.OfficialTariffDocument(
        supplier="cez", source_url=candidate.document.source_url,
        discovered_at=candidate.document.discovered_at, document_date=candidate.document.document_date,
        sha256=sha256, content_type="application/pdf",
    )
    validated = download_module.ValidatedTariffDownload(
        ...
        candidate=candidate, document=document, content=content,
        validated_at=datetime(2026, 8, 14, 16, 46, tzinfo=timezone.utc),
    )
    parsed = parser_preview.SupplierTariffParsePreview(
        supplier="cez", product_name="Basic", valid_from=date(2026, 1, 1), distribution_tariff="D25d",
        high_rate_czk_per_kwh=Decimal("3.96"), low_rate_czk_per_kwh=Decimal("3.70"),
        supplier_standing_czk_month=Decimal("130.68"), includes_vat=True,
        source_url=document.source_url, document_sha256=sha256, page_count=2,
        parser_name="cez_commercial_v1", extraction_method="pypdf_layout", extraction_confidence=100,
    ...
    regulated = regulated_pricing.RegulatedTariffBundle(
        distributor="cez_distribuce", distribution_tariff="D25d", breaker_code=breaker,
        valid_from=regulated_valid_from, valid_to=regulated_valid_to,
        variable_components=(
        ...
        source_url=regulated_url, document_date=date(2025, 11, 28), checksum=regulated_checksum, confirmed=confirmed,
    )
    evidence = (provenance.PriceEvidence(
        scope=sources.PRICE_SCOPE_REGULATED, source_name="Energetický regulační úřad",
        document_name="Cenový výměr 14/2025 – fixture", source_url=regulated_url,
        valid_from=regulated_valid_from, valid_to=regulated_valid_to,
        document_date=date(2025, 11, 28), checksum=regulated_checksum,
    ),)
...
def test_regulated_evidence_must_match_bundle_source_and_checksum() -> None:
    ...
    wrong_source = (provenance.PriceEvidence(
        scope=sources.PRICE_SCOPE_REGULATED, source_name="ERÚ", document_name="wrong source",
        source_url="https://eru.gov.cz/other-official-page", valid_from=date(2026, 1, 1), valid_to=date(2026, 12, 31), checksum="b" * 64,
    ),)
    ...
    wrong_checksum = (provenance.PriceEvidence(
        scope=sources.PRICE_SCOPE_REGULATED, source_name="ERÚ", document_name="wrong checksum",
        source_url=regulated.source_url, valid_from=date(2026, 1, 1), valid_to=date(2026, 12, 31), checksum="c" * 64,
    ),)
...
def test_non_overlapping_regulated_validity_fails_closed() -> None:
    _sources, _provenance, all_in, contract, validated, parsed, regulated, evidence = _fixture(
        regulated_valid_from=date(2025, 1, 1), regulated_valid_to=date(2025, 12, 31)
    )
