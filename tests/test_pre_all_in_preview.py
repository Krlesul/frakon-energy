...
def test_pre_supplier_preview_combines_only_with_independent_confirmed_regulation() -> None:
    (
        contracts,
        pricing,
        sources,
    ...
    candidate = sources.TariffDocumentCandidate(
        document=sources.OfficialTariffDocument(
            supplier="pre",
            source_url=(
                "https://www.pre.cz/cs/linky/dokumenty-ke-stazeni/cenik/"
                "elektrina/pre/moo/pre-proud-nefix/"
            ),
            discovered_at=datetime(2026, 8, 15, 7, 50, tzinfo=timezone.utc),
        ...
        valid_to=None,
        match_score=100,
        match_reasons=("exact verified PRE fixture",),
    ...
    validated = download_module.ValidatedTariffDownload(
        ...
        document=document,
        content=content,
        validated_at=datetime(2026, 8, 15, 7, 51, tzinfo=timezone.utc),
    )
    parsed = parser_preview.SupplierTariffParsePreview(
        ...
        valid_from=date(2026, 1, 1),
        distribution_tariff="D25d",
        high_rate_czk_per_kwh=Decimal("4.235"),
        low_rate_czk_per_kwh=Decimal("3.993"),
        supplier_standing_czk_month=Decimal("143.99"),
        includes_vat=True,
        source_url=document.source_url,
        document_sha256=sha256,
        page_count=2,
        parser_name="pre_commercial_v1",
        extraction_method="pypdf_layout",
        extraction_confidence=100,
        validation_reasons=(
            "exact PRE supplier-commercial fixture",
            "regulated supplier-PDF rows excluded",
        ),
    )

    regulated_checksum = "c" * 64
    ...
    evidence = (
        provenance.PriceEvidence(
            ...
            document_name="Cenový výměr 14/2025 – PRE fixture",
            source_url=regulated_url,
            valid_from=date(2026, 1, 1),
            valid_to=date(2026, 12, 31),
            document_date=date(2025, 11, 28),
    ...
    assert payload["supplier"] == "pre"
    assert payload["product_name"] == "PRE PROUD NEFIX"
    assert payload["all_in_vt_czk_kwh"] == "5.600243"
    assert payload["all_in_nt_czk_kwh"] == "4.753243"
    assert payload["fixed_monthly_total_czk"] == "401.5627"
    assert payload["variable_components"][0]["name"] == "PRE – obchodní cena elektřiny"
    assert payload["fixed_components"][0]["name"] == "PRE – stálá platba dodavatele"
    ...
    assert supplier_evidence["source_name"] == "Pražská energetika"
