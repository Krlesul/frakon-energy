...
def test_tariff_diagnostics_websocket_is_imported_once() -> None:
    tree = _tree()
    imports = [
        node
        for node in tree.body
        ...
        and node.level == 1
        and node.module == "tariff_diagnostics_ws_api"
    ]
    assert len(imports) == 1
    assert [alias.name for alias in imports[0].names] == [
        "async_register_tariff_diagnostics_websocket"
    ]


def test_tariff_diagnostics_is_after_confirmed_cost_authority_and_before_source_or_load_workflows() -> None:
    ...
    assert len(customer) == len(daily) == len(diagnostics) == len(mnd_source) == len(load_plan) == 1
    assert customer[0] < daily[0] < diagnostics[0] < mnd_source[0] < load_plan[0]
